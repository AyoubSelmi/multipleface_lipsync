...
def crop_video(pyframesPath,cropScale, audioFilePath,nDataLoaderThread, track, cropFile):
	...
	command = ("ffmpeg -y -i %st.avi -i %s -threads %d -c:v copy -c:a copy %s.avi -loglevel panic" % \
			  (cropFile, audioTmp, nDataLoaderThread, cropFile)) # Combine audio and video file
...
def get_asd_frames(vidTracks,scores,pyframesPath):
	...
	for fidx, fname in tqdm.tqdm(enumerate(flist), total = len(flist)):		
		image = cv2.imread(fname)
		for face in faces[fidx]:
			coordinates=int(face['x']-face['s']),int(face['y']-face['s']),int(face['x']+face['s']), int(face['y']+face['s'])
...
def frames_asd(videoFolder,videoName,outputFolder):
	...
	command = ("ffmpeg -y -i %s -qscale:v 2 -threads %d -f image2 %s -loglevel panic" % \
		(videoFilePath, nDataLoaderThread, os.path.join(pyframesPath, '%06d.jpg'))) 
	subprocess.call(command, shell=True, stdout=None)
	...
	for shot in scene:
		if shot[1].frame_num - shot[0].frame_num >= minTrack: # Discard the shot frames less than minTrack frames
			allTracks.extend(track_shot(numFailedDet, minTrack, minFaceSize, faces[shot[0].frame_num:shot[1].frame_num])) # 'frames' to present this tracks' timestep, 'bbox' presents the location of the faces
	sys.stderr.write(time.strftime("%Y-%m-%d %H:%M:%S") + " Face track and detected %d tracks \r\n" %len(allTracks))

	# Face clips cropping
	for ii, track in tqdm.tqdm(enumerate(allTracks), total = len(allTracks)):
		vidTracks.append(crop_video(pyframesPath,cropScale, audioFilePath,nDataLoaderThread, track, os.path.join(pycropPath, '%05d'%ii)))

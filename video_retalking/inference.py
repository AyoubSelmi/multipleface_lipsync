...
def datagen(
    frames, mels, full_frames, frames_pil, all_coordinates, output_folder, base_name
):
    ...
    lms = kp_extractor.extract_keypoint(
        fr_pil, os.path.join(output_folder, "temp/", base_name + "x12_landmarks.txt")
    )
    frames_pil = [
    ...
    for inverse_transform, crop, full_frame, face_det, coordinates in zip(
        inverse_transforms, crops, full_frames, face_det_results, all_coordinates
    ):
        oy1 = coordinates[1]
        oy2 = coordinates[3]
        ox1 = coordinates[0]

...
# expression control
from video_retalking.third_part.ganimation_replicate.model.ganimation import (
    GANimationModel,
)

from video_retalking.utils import audio
from video_retalking.utils.ffhq_preprocess import Croper
from video_retalking.utils.alignment_stit import (
    crop_faces,
    calc_alignment_coefficients,
    paste_image,
)
from video_retalking.utils.inference_utils import (
...
def main(video_path, audio_path, output_folder, outfile):
    ...
    enhancer = FaceEnhancement(
        base_dir="video_retalking/checkpoints",
        size=512,
        model="GPEN-BFR-512",
        use_sr=False,
    ...
    restorer = GFPGANer(
        model_path="video_retalking/checkpoints/GFPGANv1.3.pth",
        upscale=1,
        arch="clean",
        channel_multiplier=2,
        bg_upsampler=None,
    ...
    frames_pil = [
        Image.fromarray(
            cv2.resize(
                frame[coordinates[1] : coordinates[3], coordinates[0] : coordinates[2]],
                (256, 256),
            )
        )
        for (frame, coordinates, _) in asd_output
    ]

    # get the landmark according to the detected face.
    if not os.path.isfile(
        os.path.join(output_folder, "temp/", base_name + "_landmarks.txt")
    ):
    ...
    if not os.path.isfile(
        os.path.join(output_folder, "temp/", base_name + "_coeffs.npy")
    ):
    ...
    expression = torch.tensor(
        loadmat("video_retalking/checkpoints/expression.mat")["expression_center"]
    )[0]
    ...
    if not os.path.isfile(
        os.path.join(output_folder, "temp/", base_name + "_stablized.npy")
    ):
        imgs = []
        for idx in tqdm(
            range(len(frames_pil)), desc="[Step 3] Stabilize the expression In Video:"
        ):
        ...
        np.save(
            os.path.join(output_folder, "temp/", base_name + "_stablized.npy"), imgs
        )
        del D_Net
    else:
    ...
    out = cv2.VideoWriter(
        "{}/temp/result.mp4".format(output_folder),
    ...
    for i, (img_batch, mel_batch, frames, coords, img_original, f_frames) in enumerate(
        tqdm(
            ...
            total=int(
                np.ceil(float(len(mel_chunks)) / lipsync_options.LNet_batch_size)
            ),
        )
    ):
    ...
    command = "ffmpeg -loglevel error -y -i {} -i {} -strict -2 -q:v 1 {}".format(
        audio_path, "{}/temp/result.mp4".format(output_folder), outfile
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Lipsync of a speaker in the presence of many faces in the video"
    )
    parser.add_argument("--videoPath", type=str, help="Full path of input video")
    parser.add_argument(
        "--audioPath", type=str, help="Full path for audio to be used for lipsync"
    )
    parser.add_argument(
        "--outputFolder",
        type=str,
        help="Full path for the folder to be used for temporary files and output video",
    )
